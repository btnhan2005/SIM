...
close all;
M = 64; %% 64-QAM
k = log2(M);    %% k-bits/symbol
nSym = 100; % Số lượng symbol
...
%% Alatouni STBC
STBCcode = zeros(2, length(qamSig));
STBCcode(:, 1:2:end) = (1/sqrt(2)) * reshape(qamSig, 2, []);
STBCcode(:, 2:2:end) = (1/sqrt(2)) * (kron(ones(1,length(qamSig)/2),[-1;1]).*flipud(reshape(conj(qamSig),2,[])));
%% OFDM
ofdmTx1 = ofdmmod(reshape(STBCcode(1,:), nfft, []), nfft, cplen);    %% ofdm
ofdmTx2 = ofdmmod(reshape(STBCcode(2,:), nfft, []), nfft, cplen);    %% ofdm
%% Kênh truyền
sigma2 = 10^(-104/10);
c = 3*10^8;
f0 = 28*10^9;
